...
function findKeyWord(node_list, keyword) {
  ...
  for (let i = 0; i < len; i++) {
    setTimeout(() => {
      ...
      if (node_list[i]['characters'].indexOf(keyword) > -1) {
        // 找到关键词
        ...
        if (node_list[i].locked == true) {
          ancestor_isLocked = true
        }
        if (node_list[i].visible == false) {
        ...
        if (ancestor_isVisible == false || ancestor_isLocked == true) {
          ...
          // 获取祖先元素的状态
          this_parent = node_list[i].parent
        ...
        data_item = { 'node': node_list[i], 'ancestor_isVisible': ancestor_isVisible, 'ancestor_isLocked': ancestor_isLocked, 'ancestor_type': ancestor_type }
