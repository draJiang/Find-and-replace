  ...
  }>
{
  ...
  render() {
    ...
    if (this.props['list_state'] == 'find') {
      ...
      let list_status 
      if (this.props['find_loading']){
        // 加载状态：显示 loading、置灰搜索结果
        modal_status = "display:block"
        list_status = "find_result_list list_disable"
      }else{
        modal_status = "display:none"
        list_status = "find_result_list"
      }

      return (
        <div>
          <div className="modal" style={modal_status}>
            < div className=" icon icon--spinner icon--spin " > </ div >
          </div>
          <div className={list_status}>{listItems}</div>
        </div>
      )
    }
