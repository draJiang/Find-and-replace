...
function myFindTextAll(node, node_list) {
  ...
  // 如果目标图层本身就是 TEXT 图层
  if (node.type == 'TEXT') {
    node_list.push({ 'node': node, 'locked': node.locked })
    return node_list
  }
  ...
  for (var i = 0; i < thisChildren.length; i++) {
    ...
    if (thisChildren[i].type == 'TEXT') {
      ...
      node_list.push({ 'node': thisChildren[i], 'locked': node.locked })
    } else {
      // 如果不是 TEXT 图层
      // 如果包含子图层
...
function find(data) {
  ...
  if (selection.length == 0) {
    ...
    node_list = myFindTextAll(figma.currentPage.children, node_list)
  } else {
    // 当前有选中图层，则在选中的图层中搜索
    // 在当前选中的图层中，搜索文本图层
