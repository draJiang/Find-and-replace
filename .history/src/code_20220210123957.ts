      ...
      if (item['node'].hasMissingFont) {
        ...
        while (true) {
          ...
          if (index > -1) {
            ...
            let oldIndentation = item['node'].getRangeIndentation(index,insertStart-1)
            let oldListOptions = item['node'].getRangeListOptions(index,insertStart-1)
            ...
            item['node'].setRangeIndentation(index,index+newCharacters.length,oldIndentation)
            // 设置序号
