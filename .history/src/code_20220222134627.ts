      ...
      if (element['characters'].indexOf(keyword) > -1) {
        ...
        while (true) {
          ...
          if (index > -1) {
            ...
            let temp = { 'type': 'find','find_end':false}, 'target_Text_Node': [{ 'id': data_item['node'].id, 'characters': data_item['node'].characters, 'start': index, 'end': index + keyword.length, 'hasMissingFont': data_item['node'].hasMissingFont,'ancestor_type':data_item['ancestor_type']] }
            if (posted.length<10) {
              figma.ui.postMessage()
              posted.push({ 'type': 'find', 'target_Text_Node': [{ 'id': data_item['node'].id, 'characters': data_item['node'].characters, 'start': index, 'end': index + keyword.length, 'hasMissingFont': data_item['node'].hasMissingFont,'ancestor_type':data_item['ancestor_type'] }] })
            }else{
