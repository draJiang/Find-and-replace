      ...
      if (node['characters'].indexOf(keyword) > -1) {
        ...
        while (index>=0) {
          ...
          if (index > -1) {
            ...
            data_temp = { 'id': node.id, 'characters': node.characters, 'start': index, 'end': index + keyword.length, 'hasMissingFont': node.hasMissingFont, 'ancestor_type': data_item['ancestor_type'] }
            if (req_cout < 10) {
