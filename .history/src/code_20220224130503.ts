...
function findKeyWord(node_list, keyword) {
  ...
  for (let i = 0; i < len; i++) {
    setTimeout(() => {
      ...
      figma.ui.postMessage({ 'type': 'loading','my_progress':{'index':i,'total':node_list.length} });
      ...
      if (node['characters'].indexOf(keyword) > -1) {
        ...
        while (index>=0) {
          ...
          if (index > -1) {
            ...
            figma.ui.postMessage({ 'type': 'loading', 'find_end': false,'progress':{'index':i,'toal':len}})
