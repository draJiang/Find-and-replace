      ...
      if (item['node'].hasMissingFont) {
        ...
        textStyle.forEach(element => {
          ...
          while (true) {
            ...
            console.log('（段落）element index:'+index.toString());
            console.log('（图层）item index:'+item['node'].characters.indexOf(data.data.keyword, position).toString());
          ...
          styleTemp.push({'start':element['start'] + offsetStart,'end':element['end'] + offsetEnd,'indentation':element['indentation'] > 0 ? element['indentation'] - 1 : element['indentation'],'listOptions':element['listOptions']})
        ...
        styleTemp.forEach(element => {
          item['node'].setRangeIndentation(element['start']+1, element['end'], element['indentation'])
          item['node'].setRangeListOptions(element['start']+1, element['end'], element['listOptions'])
        });
