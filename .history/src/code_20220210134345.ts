    ...
    if (item['ancestor_isVisible'] == false || item['ancestor_isLocked'] == true) {
      ...
      if (item['node'].hasMissingFont) {
        ...
        while (true) {
          ...
          // 获取匹配到的字符的索引
          var index = item['node'].characters.indexOf(data.data.keyword, position)
          ...
          if (index > -1) {
            ...
            // 在索引后插入新字符
            let insertStart = index + data.data.keyword.length
            let newCharacters = data.data.replace_word

            let oldIndentation = item['node'].getRangeIndentation(index, insertStart)
            let oldListOptions = item['node'].getRangeListOptions(index, insertStart)
            console.log(item['node'].getRangeIndentation(index, insertStart));
            ...
            // 根据索引删除旧字符
            item['node'].deleteCharacters(index, insertStart)
            console.log('getRangeIndentation:');
            console.log(item['node'].getRangeIndentation(index, insertStart));
            ...
            textStyle.forEach(element => {
              ...
              let offsetStart,offsetEnd = 0

              // 替换前的文字和替换后的长度不一致
              if (data.data.keyword.length != newCharacters.length) {
                // 设置偏移，以确保样式设置正确
                ...
                // 如果修改点在文本段落中间
                if (element['start'] <= index && element['end'] >= insertStart) {
                  offsetStart += 0
                  offsetEnd += data.data.keyword.length - newCharacters.length
                }
              ...
              // 设置缩进
              item['node'].setRangeIndentation(element['start'] + offset, element['end'] + offset, element['indentation'] > 0 ? element['indentation'] - 1 : element['indentation'])
              // 设置序号
              item['node'].setRangeListOptions(element['start'] + offset, element['end'] + offset, element['listOptions'])
