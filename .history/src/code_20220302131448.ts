...
figma.ui.onmessage = msg => {
  ...
  if (msg.type === 'search') {
    ...
    setTimeout(() => {
      ...
      // 在文本图层中匹配包含关键字的图层
      toHTML = findKeyWord(node_list, msg.data.keyword).then((res)=>{
        
        // 将搜索数据发送给 ui.tsx
        console.log('findKeyWord res:');
        
        console.log(res);
        console.log(toHTML);
        
        figma.ui.postMessage({ 'type': 'find', 'done': true, 'target_Text_Node': res })
...
// 在文本图层中，匹配关键字
async function findKeyWord(node_list, keyword) {
