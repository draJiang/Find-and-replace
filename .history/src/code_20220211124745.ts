    ...
    if (item['ancestor_isVisible'] == false || item['ancestor_isLocked'] == true) {
      ...
      if (item['node'].hasMissingFont) {
        ...
        textStyle.forEach(element => {
          ...
          styleTemp.push({'start':element['start'] + offsetEnd,'end':element['end'] + offsetEnd,'indentation':element['indentation'] > 0 ? element['indentation'] : element['indentation'],'listOptions':element['listOptions']})
