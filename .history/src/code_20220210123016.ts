      ...
      if (item['node'].hasMissingFont) {
        ...
        while (true) {
          ...
          if (index > -1) {
            ...
            item['node'].insertCharacters(insertStart,newCharacters,'AFTER')
            // 根据索引删除旧字符
            ...
            // 设置序号
            item['node'].setRangeIndentation(index,insertStart,oldIndentation)
            item['node'].setRangeListOptions(index,insertStart,oldListOptions)
